...
- Generic-door source row 103 (`udp1_ohfire2` / `t_door93`) is withheld because no `t_door93.dwk` exists in any reviewed source; its appearance ID remains an explicit blank row so every later door keeps its original ID. The model remains staged in `srn_door` pending a verified walkmesh.
...
|---|---|---|---|
| D20 Network Interior | net01 | malformed secondary-door coordinates; unavailable custom minimaps; source exposed unsupported height controls | height controls disabled; optional door/minimap polish |
| D20 TBX Interior | tbx78 | unavailable `tbx79` minimaps; seven incomplete `tbx79` WOK wrappers | WOK wrappers/roots repaired; optional minimap polish |
| D20 Secret Base | tjsb0 | two suspect per-tile minimap selections | ownership fixed; optional per-tile polish |
| D20 Vacuum Exterior | vac01 | duplicate Asteroid2 group label, nebula walkmesh, minimaps, and shooting-star animation name | Asteroid3 label repaired; optional visual/pathing polish |
| D20 Modern Exterior | dgt04 | incomplete WOK wrappers plus mismatched edge, terrain-corner, preview, and shared-texture metadata | WOK/model/edge/group/preview records repaired; shared chain-link texture landed in `srn_t_common`; optional terrain/minimap polish |
...
| D20 Parking Garage | flow_pa | group-centric legacy SET: four exposed terrain/crosser brushes have no placement rules; palette-prefix mismatch and unavailable minimaps | usable through supplied groups; optional rule/palette/minimap polish |
| D20 ServiceDucts | nac01 | one malformed door coordinate and crosser mismatch | preserve source; optional door/terrain polish |
| D20 SJ Metal Interior | sjm01 | one mismatched minimap; two door slots reference models whose source DWKs do not exist | minimap repaired; door rows withheld as blank IDs and affected slots disabled; models retained for later recovery |
| D20 Shadowrun Exterior | srt04 | shared DGT door, crosser, model, and minimap metadata diverged from the packaged source | matching DGT metadata restored; optional palette/terrain polish |
...
| D20 Office Interiors UDP | udp2 | water-cooler texture-name mismatch; legacy edge table names absent `gi_z*` models and lacks Office_Alum rules | usable without edge polish; preserve source behavior |
| D20 Planetscape | vmp01 | source disabled declared height transitions, used mismatched edge aliases and palette ID; Wall Gate is source-confirmed decorative | height, edge, and palette metadata repaired; use EE `env_tti`; preserve source gate behavior |
| D20 Mars Interior | vmr01 | one mismatched WOK identity, all 191 custom minimaps, and fifteen edge models absent from inspected source HAKs | WOK identity repaired; usable with blank area-map tiles; optional minimap/edge recovery |
| D20 ZSF Interior | zsf01 | invalid light flags, global minimap-prefix and tile-34 mismatches, stair visibility, and unavailable WOK/edge/texture assets | flags, minimaps, and stair visibility repaired; preserve remaining source gaps for optional polish |
