...
if ($Mode -eq 'Analyze') {
    ...
    foreach ($file in $rawFiles | Sort-Object Name) {
        ...
        $records.Add([pscustomobject]@{
            name = $name; type = $file.Extension.TrimStart('.').ToLowerInvariant(); size = $file.Length
...
if (($profile.PSObject.Properties.Name -contains 'genericDoorMerge') -or
    ...
    ($profile.PSObject.Properties.Name -contains 'indexedResourceTableMerges')) {
...
if ($profile.PSObject.Properties.Name -contains 'indexedResourceTableMerges') {
    foreach ($merge in @($profile.indexedResourceTableMerges)) {
        ...
        $columnCount = @($header[0].Trim() -split '\s+').Count
        ...
        foreach ($mapping in @($mappings | Sort-Object targetRow)) {
            ...
            $targetTokens = @($baseLines[$targetIndex].Trim() -split '\s+')
            if (@($targetTokens | Select-Object -Skip 1 | Where-Object { $_ -ne '****' }).Count) {
            ...
            $sourceTokens = @($sourceLine[0].Trim() -split '\s+')
            $globalAllocations.Add([pscustomobject]@{
                table = $resource; sourceRow = [int]$mapping.sourceRow; targetRow = [int]$mapping.targetRow
                label = $sourceTokens[1]; resource = $sourceTokens[2]; pack = [string]$merge.outputPack
            })
        }
...
if ($null -ne $mergedDoorLines) {
    ...
    [IO.File]::WriteAllLines((Join-Path $repoRoot 'srn_2da\doortypes.2da'), $mergedDoorLines, [Text.Encoding]::GetEncoding(1252))
}
...
$configuration.HakList = $registered; [IO.File]::WriteAllText($configPath, ($configuration | ConvertTo-Json -Depth 32) + "`n", [Text.UTF8Encoding]::new($false)); Write-Host 'Apply complete.'
